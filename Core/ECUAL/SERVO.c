...
void SERVO_Init(uint16_t au16_SERVO_Instance)
{
	...
	else if(SERVO_CfgParam[au16_SERVO_Instance].TIM_Instance == TIM8)
	{
		GPIO_InitStruct.Alternate = GPIO_AF3_TIM8;
	}
	else if(SERVO_CfgParam[au16_SERVO_Instance].TIM_Instance == TIM9)
	{
	...
	else if(SERVO_CfgParam[au16_SERVO_Instance].TIM_Instance == TIM13)
	{
...
void SERVO_MoveTo(uint16_t au16_SERVO_Instance, float af_Angle)
{
	...
	au16_Pulse = ((af_Angle*(gs_SERVO_info[au16_SERVO_Instance].Period_Max - gs_SERVO_info[au16_SERVO_Instance].Period_Min))/180.0)+ gs_SERVO_info[au16_SERVO_Instance].Period_Min;

	*(SERVO_CfgParam[au16_SERVO_Instance].TIM_CCRx) = (uint16_t)au16_Pulse;
}
